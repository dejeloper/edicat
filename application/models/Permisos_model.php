...
class Permisos_model extends CI_Model {
    ...
    public function obtenerSuperUsuarios($usuario) {
        $this->db->where('Usuario', $usuario);
        $this->db->where('Habilitado', 1);
        $query = $this->db->get("SuperUsuarios");
        if ($query->num_rows() <= 0) {
            return false;
        } else {
            return $query->result_array();
    ...
    public function saveLogAccesosDenegados($data) {
        if ($this->db->insert("LogAccesosDenegados", $data)) {
            return $error = $this->db->error();
        } else {
            return 1;
        }
